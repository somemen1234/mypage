...
const $submitButton = document.querySelector("#submitButton"); //id
...
function makeAnswerCircle(correct) {
  const CIRCLE_NAME = "answer"
  showCircle(correct, CIRCLE_NAME, $answerCircleArea).then(div => {
    div.id = "answerCircle";
...
function makeGuessCircle(guess) {
  ...
  }) //Promise를 반환하기 때문에 .then으로 성공을 했을 경우를 처리 가능
  // 구현3. 유저가 원의 크기로 정답을 유추하기 쉽도록 showCircle 함수를 이용해서 해당 부분을 구현해주세요,
  // showCircle 함수의 "작업"이 끝나면, 해당 div에 원의 이름을 입력해주세요
}
...
function showCircle(size, circleName, area) {
  ...
  return new Promise((resolve) => {         //0.01초를 기다린다음 그린다.
    setTimeout(() => {
